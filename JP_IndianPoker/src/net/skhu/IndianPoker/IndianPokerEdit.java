...
public class IndianPokerEdit extends Thread {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		while(round <= 20) {
			if(user.getCoin() < 1 || ai.getCoin() < 1) {
				...
				System.out.println("[ai ● : "+ai.getCoin()+"] [user ● : "+user.getCoin()+"]");
			...
			Indian : if(flag % 2 == 0) {
				...
				// user 초기 배팅 선택 (if)
				if(choice == 1) {
					System.out.print("배팅할 ● 개수를 입력하세요: ");
					...
					if(ai.getChoice() == 1) {
						...
						aiSum += aiBet;
						Thread.sleep(3500);
						System.out.println("[ai 배팅 ● : "+aiBet+"] [user 배팅 ● : "+userBet+"]");
					}
					// ai 배팅 선택 (if) 끝
					// ai 배팅 포기 (else)
					else {
						...
						System.out.println("             └---┘");
						System.out.println("               ┌---┐");
						System.out.println(" user의 카드 : │ " + user.getCard(flag) + " │");
						System.out.println("               └---┘");
						...
						System.out.print("다음 라운드로 넘어가시려면 [1]을 입력하세요: ");
					...
					while(userSum != aiSum) {
						...
						addBet : if(userSum > aiSum) {
							...
							if(ai.getChoice() == 1) {
								...
								aiSum += aiBetAgain;
								Thread.sleep(3500);
								System.out.println("[ai 추가 배팅 ● : "+aiBetAgain+"]");
								System.out.println("[userSum = "+userSum+"] [aiSum = "+aiSum+"]");
								break addBet;
							}
							// ai 추가 배팅 선택 (if) 끝
						...
						// userSum이 더 작은 경우 (if)
							...
							if(choiceAgain == 1) {
								...
								userBet = scan.nextInt();
								userSum += userBet;
								System.out.println("[user 추가 배팅 ● : "+userBet+"]");
								System.out.println("[userSum = "+userSum+"] [aiSum = "+aiSum+"]");
								break addBet;
							}
							// user 추가 배팅 선택 (if) 끝
			...
			// ai 선 배팅 판단 flag%2 == 1 (else)
				...
				if(ai.getChoice() == 1) {
					...
					aiSum += aiBet;
					Thread.sleep(3500);
					System.out.println("[ai 배팅 ● : "+aiBet+"]");
					System.out.print("배팅은 [1] 포기는 [2]를 입력하세요: ");
					int choice = scan.nextInt();
					// user 배팅 선택 (if)
					if(choice == 1) {
